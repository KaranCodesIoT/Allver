...
export default function ProfileScreen() {
  ...
  const pickHighlightMedia = async (type: 'image' | 'video') => {
    ...
    if (!result.canceled && result.assets && result.assets[0].uri) {
      handleUploadHighlightMedia(result.assets[0].uri, type);
    }
  };

  const handleUploadHighlightMedia = async (localUri: string, type: 'image' | 'video') => {
    setUploadingMedia(true);
    try {
      const formData = new FormData();
      
      // On web/mobile React Native FormData file append helper
      ...
      if (res.ok) {
        ...
        if (data.url) {
          ...
          Alert.alert('Success', `${type === 'image' ? 'Photo' : 'Video'} selected successfully!`);
        }
      } else {
        Alert.alert('Error', 'Failed to upload media file.');
      }
    } catch (err) {
      console.error(err);
  ...
  const handleSubmitHighlight = async () => {
    ...
    if (!formMediaUrl) {
      Alert.alert('Error', 'Please select and upload an image or video.');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Refresh User Data (which loads highlights)
        loadUserData();
      } else {
                    ...
                    return (
                      <View key={item._id || index} style={styles.highlightItemContainer}>
                        <TouchableOpacity 
                          ...
                          onPress={() => router.push({ pathname: '/portfolio-highlights', params: { userId: user.id } })}
                          activeOpacity={0.85}
                        >
                          {isVideo ? (
                            <Video 
      ...
      >
        <View style={styles.modalOverlay}>
          <View style={[styles.modalContent, { width: width * 0.9, padding: 20 }]}>
            ...
            {/* Uploading indicator or file selected status */}
            {uploadingMedia && (
            ...
            {formMediaUrl && !uploadingMedia && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: '#ECFDF5', borderColor: '#A7F3D0', borderWidth: 1, borderRadius: 8, padding: 10, marginBottom: 16 }}>
                <Feather name="check-circle" size={16} color="#10B981" />
                <Text style={{ fontSize: 12, color: '#065F46', fontWeight: '500', flex: 1 }} numberOfLines={1}>
