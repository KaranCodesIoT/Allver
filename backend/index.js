...
require('./models/ProjectWorkspace');
require('./models/Notification');
require('./models/CallHistory');
require('./models/Ledger');
require('./models/CommissionConfig');
...
io.use(async (socket, next) => {
  // 1. Connection Rate Limiting: Allow up to 300 attempts per minute to handle dev reloads and mobile reconnects
  ...
  if (limitData.count > 300) {
    console.warn(`[Socket RateLimit] IP ${ip} exceeded connection threshold (${limitData.count}/min)`);
    return next(new Error("Too many connection attempts. Please try again later."));
  }

  // 2. Flexible token/userId resolution from auth or query
  const rawTokenOrId = socket.handshake.auth?.token || socket.handshake.query?.token || socket.handshake.query?.userId || socket.handshake.auth?.userId;
  let resolvedUserId = null;

  if (rawTokenOrId) {
    if (mongoose.Types.ObjectId.isValid(rawTokenOrId) && String(rawTokenOrId).length === 24) {
      resolvedUserId = String(rawTokenOrId);
    } else {
      try {
        const decoded = jwt.decode(rawTokenOrId);
        if (decoded && (decoded.userId || decoded.id || decoded._id)) {
          resolvedUserId = String(decoded.userId || decoded.id || decoded._id);
        }
      } catch (e) {}
    }
  }
  ...
  if (!resolvedUserId) {
    const explicitId = socket.handshake.query?.userId || socket.handshake.auth?.userId;
    if (explicitId && mongoose.Types.ObjectId.isValid(explicitId)) {
      resolvedUserId = String(explicitId);
    }
  }

  socket.userId = resolvedUserId || rawTokenOrId || socket.id;
  next();
});
...
io.on('connection', (socket) => {
  ...
  if (socket.userId) {
    ...
    // Automatically hydrate worker GPS location and skills from DB into activeWorkerLocations
    if (mongoose.Types.ObjectId.isValid(socket.userId)) {
      const User = mongoose.model('User');
      User.findById(socket.userId).lean().then(userDoc => {
        if (userDoc && (userDoc.role === 'Labour' || userDoc.role === 'Contractor' || userDoc.role === 'Worker')) {
          const isAvail = userDoc.availability !== 'Not Available' && userDoc.isAvailableForBooking !== false;
          if (userDoc.latitude && userDoc.longitude) {
            activeWorkerLocations.set(socket.userId.toString(), {
              userId: socket.userId.toString(),
              fullName: userDoc.fullName,
              socketId: socket.id,
              isOnline: true,
              latitude: Number(userDoc.latitude),
              longitude: Number(userDoc.longitude),
              formattedAddress: userDoc.formattedAddress || userDoc.location || '',
              workArea: userDoc.workArea || '',
              availability: isAvail ? 'Available' : 'Not Available',
              isAvailableForBooking: isAvail,
              skillType: userDoc.skillType || '',
              specialization: userDoc.specialization || [],
              workCategory: userDoc.workCategory || [],
              role: userDoc.role || 'Labour',
              serviceRadiusKm: userDoc.serviceRadiusKm || userDoc.workAreaRadius || 10,
              lastLocationUpdate: userDoc.lastLocationUpdate || new Date(),
              updatedAt: userDoc.lastLocationUpdate ? new Date(userDoc.lastLocationUpdate).getTime() : Date.now()
            });
            console.log(`[WorkerLocation] Hydrated active worker "${userDoc.fullName}" (${socket.userId}) on socket connection: (${userDoc.latitude}, ${userDoc.longitude}) - Skill: "${userDoc.skillType}" - Available: ${isAvail}`);
          }
        }
      }).catch(err => console.error('[WorkerLocation] Hydration error:', err));
    }
  }
  ...
  socket.on('disconnect', () => {
    clearInterval(heartbeatInterval);
    if (socket.userId && activeWorkerLocations.has(socket.userId.toString())) {
      const existing = activeWorkerLocations.get(socket.userId.toString());
  ...
  socket.on('join_room', async ({ roomId }) => {
    ...
    // Allow custom prefixed rooms (job:*, chat:*, user:*)
    if (typeof roomId === 'string' && (roomId.startsWith('job:') || roomId.startsWith('chat:') || roomId.startsWith('user:'))) {
      socket.join(roomId);
      console.log(`[Socket] User ${uId} joined prefixed room: "${roomId}"`);
      return;
    }
    ...
    try {
      ...
      // 2. Check if it's a Conversation (one-to-one DM or active Job chat)
      const conversation = await Conversation.findById(roomId).lean();
      if (conversation) {
        socket.join(roomId);
        socket.join(`chat:${roomId}`);
        console.log(`[Socket] User ${uId} joined Conversation room: ${roomId} and chat:${roomId}`);
        return;
      }
  ...
  socket.on('go_online', () => {
    ...
    // Update in-memory worker online state
    const activeWorkerLocations = global.activeWorkerLocations || (global.activeWorkerLocations = new Map());
    const wInfo = activeWorkerLocations.get(userId.toString());
    if (wInfo) {
      wInfo.isOnline = true;
      wInfo.socketId = socket.id;
      wInfo.updatedAt = Date.now();
    }
  ...
  // ========== LOCATION-BASED WORKER MATCHING & DISPATCH ENGINE ==========
  const BookingDispatchEngine = require('./services/BookingDispatchEngine');
  const activeWorkerLocations = global.activeWorkerLocations || (global.activeWorkerLocations = new Map());
  const bookingDispatchEngine = global.bookingDispatchEngine || (global.bookingDispatchEngine = new BookingDispatchEngine(io, activeWorkerLocations));

  // Configurable service radii per trade/service
  ...
  // Normalize requested service trade into standardized category
  function normalizeServiceTrade(serviceName) {
  ...
  function getServiceRadiusConfig(serviceName) {
    const trade = normalizeServiceTrade(serviceName);
    return SERVICE_RADIUS_CONFIG[trade] || SERVICE_RADIUS_CONFIG.default;
  }

  // Calculate actual distance between coordinates using Haversine formula (in km)
  ...
  const SERVICE_SKILL_MAP = {
    ...
    Other: ['labour', 'general worker', 'helper', 'worker']
  };

  function isWorkerMatchingService(workerSkill, workerRole, workerCategory, requestedService) {
    if (!requestedService) return true;
    const requestedTrade = normalizeServiceTrade(requestedService);
    const acceptableSkills = SERVICE_SKILL_MAP[requestedTrade] || [];
    ...
    const workerText = `${skillStr} ${catStr} ${roleStr}`.trim();

    // 1. Direct match with acceptable trade skills
    const matchSkill = acceptableSkills.some(s => workerText.includes(s.toLowerCase()));
    if (matchSkill) return true;

    // 2. Direct match with requested service string
    if (workerText.includes(requestedService.toLowerCase())) return true;

    // 3. General work allows general labour
  ...
  // Known city fallback coordinates (to resolve DB workers whose lat/lng were stored via city string)
  const KNOWN_COORDINATES_MAP = {
    ...
    'hitec city, hyderabad': { lat: 17.4474, lng: 78.3762 },
    'gachibowli, hyderabad': { lat: 17.4401, lng: 78.3489 },
    'hyderabad': { lat: 17.3850, lng: 78.4867 },
    'kothrud, pune': { lat: 18.5074, lng: 73.8077 },
    'pune': { lat: 18.5204, lng: 73.8567 },
    'salt lake, kolkata': { lat: 22.5867, lng: 88.4178 },
    'kolkata': { lat: 22.5726, lng: 88.3639 },
    't. nagar, chennai': { lat: 13.0418, lng: 80.2341 },
    'chennai': { lat: 13.0827, lng: 80.2707 },
    'navrangpura, ahmedabad': { lat: 23.0365, lng: 72.5611 },
    'ahmedabad': { lat: 23.0225, lng: 72.5714 },
    'vaishali nagar, jaipur': { lat: 26.9048, lng: 75.7480 },
    'jaipur': { lat: 26.9124, lng: 75.7873 },
    'gomti nagar, lucknow': { lat: 26.8525, lng: 81.0003 },
  ...
  // Max allowed location age (3 hours) before worker location is considered stale
  const MAX_WORKER_LOCATION_AGE_MS = 3 * 60 * 60 * 1000;

  // Live Worker Location Update Event
  socket.on('worker_update_location', async (data) => {
    const { latitude, longitude, formattedAddress, availability, isAvailableForBooking, skillType, serviceRadiusKm, workArea } = data || {};
    const userId = socket.userId;
    if (!userId) return;
    ...
    // If coordinates are not provided, resolve from workArea / address text
    if (isNaN(lat) || isNaN(lng) || (lat === 0 && lng === 0)) {
      const resolved = resolveWorkerCoordinates({ workArea, formattedAddress });
      if (resolved) {
        lat = resolved.lat;
        lng = resolved.lng;
        console.log(`[WorkerLocation] Resolved coordinates (${lat}, ${lng}) from Work Area "${workArea}" for Worker ${userId}`);
      }
    }

    const isAvailable = availability !== 'Not Available' && isAvailableForBooking !== false;

    if (!isNaN(lat) && !isNaN(lng)) {
      const existing = activeWorkerLocations.get(userId.toString()) || {};
      activeWorkerLocations.set(userId.toString(), {
        ...existing,
        ...
        skillType: skillType || existing.skillType || '',
        serviceRadiusKm: serviceRadiusKm ? Number(serviceRadiusKm) : (existing.serviceRadiusKm || 15),
        updatedAt: Date.now()
      });

      console.log(`[WorkerLocation] Live location updated for Worker ${userId}: (${lat}, ${lng}) - Work Area: "${workArea || 'None'}" - Available: ${isAvailable}`);

      // Stream live location to active travelling job if applicable
      bookingDispatchEngine.broadcastWorkerLocation(userId, lat, lng, data.jobId);
      ...
      try {
        if (mongoose.connection && mongoose.connection.readyState === 1) {
          ...
          if (formattedAddress) dbUpdate.formattedAddress = formattedAddress;
          if (workArea) dbUpdate.workArea = workArea;
          if (serviceRadiusKm) dbUpdate.serviceRadiusKm = Number(serviceRadiusKm);

          await User.findByIdAndUpdate(userId, { $set: dbUpdate }, { maxTimeMS: 5000 });
        }
      } catch (err) {
        console.error('[WorkerLocation] DB update error:', err);
      }
    } else if (availability !== undefined || isAvailableForBooking !== undefined || serviceRadiusKm !== undefined || workArea !== undefined) {
      // Non-GPS property update
      const existing = activeWorkerLocations.get(userId.toString()) || {};
      activeWorkerLocations.set(userId.toString(), {
        ...existing,
        userId: userId.toString(),
  ...
  // ========== AUTHORITATIVE STATE MACHINE DISPATCH EVENTS ==========
  socket.on('client_create_job_request', async (data) => {
    await bookingDispatchEngine.createAndStartJobRequest(socket, data);
  });

  // Worker Accepts Job (Supports both 'worker_accept_job' and 'worker_accept_job_request')
  const handleWorkerAccept = async (data) => {
    const { jobId, workerInfo } = data || {};
    await bookingDispatchEngine.acceptJob(jobId, workerInfo, socket);
  };
  socket.on('worker_accept_job', handleWorkerAccept);
  socket.on('worker_accept_job_request', handleWorkerAccept);

  socket.on('worker_reject_job_request', ({ jobId }) => {
    console.log(`[JobDispatch] Worker ${socket.userId} rejected Job ${jobId}`);
  });

  socket.on('client_cancel_job_request', async ({ jobId }) => {
    await bookingDispatchEngine.cancelJob(jobId);
  });

  // 1. Worker Starts Trip / I'm On The Way
  socket.on('worker_start_trip', async ({ jobId }) => {
    await bookingDispatchEngine.startTrip(jobId, socket);
  });

  // 2. Real-Time Worker Location Update (streaming during trip)
  socket.on('worker_location_update', async (data) => {
    const { jobId, workerId, latitude, longitude, heading, speed } = data || {};
    await bookingDispatchEngine.broadcastWorkerLocation(workerId || socket.userId, latitude, longitude, jobId, { heading, speed });
  });

  // 3. Worker Arrived at site
  socket.on('worker_arrived', async ({ jobId }) => {
    await bookingDispatchEngine.workerArrived(jobId, socket);
  });

  // 4. Worker Starts Work
  socket.on('worker_start_work', async ({ jobId }) => {
    await bookingDispatchEngine.startWork(jobId, socket);
  });

  // 5. Worker Submits Completion
  socket.on('worker_submit_completion', async ({ jobId, finalAmount, notes, photos }) => {
    await bookingDispatchEngine.submitJobCompletion(jobId, { finalAmount, notes, photos }, socket);
  });

  // 6. Client Confirms Completion
  const handleClientConfirm = async ({ jobId }) => {
    await bookingDispatchEngine.confirmJobCompletion(jobId);
  };
  socket.on('client_confirm_completion', handleClientConfirm);
  socket.on('client_confirm_job_completion', handleClientConfirm);

  // 7. Payment Processed
  const handleProcessPayment = async (data) => {
    const { jobId, method, amount, fee, total } = data || {};
    await bookingDispatchEngine.processPayment(jobId, { method, amount, fee, total });
  };
  socket.on('process_payment', handleProcessPayment);
  socket.on('client_pay_job', handleProcessPayment);

  // 8. Backwards compatibility for generic status updater
  socket.on('worker_update_job_status', async ({ jobId, status, meta }) => {
    await bookingDispatchEngine.updateJobStatus(jobId, status, meta, socket);
  });

  // 9. Ratings
  socket.on('submit_job_rating', async ({ jobId, rating, reviewText, ratedRole }) => {
    await bookingDispatchEngine.submitRating(jobId, { rating, reviewText, ratedRole });
  });

  // 10. Fetch Authoritative Job (Database + Memory)
  socket.on('get_active_job', async ({ jobId }, callback) => {
    let job = null;
    if (jobId) {
      job = await bookingDispatchEngine.getJob(jobId);
    } else if (socket.userId) {
      job = await bookingDispatchEngine.getActiveJobForUser(socket.userId);
    }
    if (typeof callback === 'function') {
      callback(job ? { success: true, job } : { success: false, reason: 'NO_ACTIVE_JOB' });
    }
  });

  // ========== NEW: PAYMENT / SETTLEMENT / WALLET SOCKET EVENTS ==========

  // 11. Client selects payment method (CASH or ONLINE)
  socket.on('job_select_payment_method', async ({ jobId, method }) => {
    const result = await bookingDispatchEngine.selectPaymentMethod(jobId, method);
    if (!result.success) {
      socket.emit('job_error', { jobId, error: result.reason });
    }
  });

  // 12. Worker confirms cash received
  socket.on('job_cash_confirmed', async ({ jobId, amount }) => {
    await bookingDispatchEngine.confirmPayment(jobId, { method: 'CASH', amount });
  });

  // 13. Client initiates online payment verification
  socket.on('job_online_payment_initiated', async ({ jobId, paymentId, orderId, signature, amount }) => {
    try {
      const ps = bookingDispatchEngine.paymentService || global.paymentService;
      if (!ps) {
        socket.emit('job_payment_failed', { jobId, reason: 'Payment service unavailable' });
        return;
      }

      let targetOrderId = orderId;
      let targetGatewayOrderId = null;
      ...
      if (!targetOrderId) {
        const order = await PaymentOrder.findOne({
          $or: [
            ...(paymentId ? [{ orderId: paymentId }, { gatewayOrderId: paymentId }] : []),
            ...(jobId ? [{ jobId, status: { $in: ['CREATED', 'PENDING', 'PAID'] } }] : [])
          ]
        }).sort({ createdAt: -1 });
        if (order) {
          targetOrderId = order.orderId;
          targetGatewayOrderId = order.gatewayOrderId;
          targetPaymentId = paymentId || order.gatewayOrderId;
        }
      }

      if (targetOrderId || targetGatewayOrderId) {
        const verification = await ps.verifyAndConfirmPayment({
          orderId: targetOrderId,
          gatewayOrderId: targetGatewayOrderId,
        ...
        if (verification.success) {
          socket.emit('job_payment_confirmed', { jobId, status: 'PAID', orderId: verification.orderId });
        } else {
          socket.emit('job_payment_failed', { jobId, reason: 'Authoritative payment verification failed' });
        }
      } else {
        await bookingDispatchEngine.confirmPayment(jobId, {
          method: 'ONLINE',
      ...
      console.error('[Socket] Online payment error:', err);
      try {
        await bookingDispatchEngine.confirmPayment(jobId, {
          method: 'ONLINE',
          paymentProviderRef: paymentId,
        ...
        socket.emit('job_payment_failed', { jobId, reason: err.message, code: err.code });
      }
    }
  });

  // 14. Worker cancels job
  socket.on('worker_cancel_job', async ({ jobId, reason }) => {
    await bookingDispatchEngine.workerCancelJob(jobId, reason);
  });

  // 15. Get wallet balance
  socket.on('get_wallet_balance', async (data, callback) => {
    const workerId = data?.workerId || socket.userId;
    if (!workerId) {
      if (typeof callback === 'function') callback({ success: false, reason: 'NO_USER_ID' });
      return;
    }
    try {
      const fs = bookingDispatchEngine.financialService;
      const wallet = await fs.getWorkerBalance(workerId);
      if (typeof callback === 'function') callback({ success: true, wallet });
    } catch (err) {
      if (typeof callback === 'function') callback({ success: false, reason: err.message });
    }
  });
  ...
  socket.on('disconnect', () => {
    if (socket.userId) {
      ...
      const activeWorkerLocations = global.activeWorkerLocations || (global.activeWorkerLocations = new Map());
      const wInfo = activeWorkerLocations.get(socket.userId.toString());
      if (wInfo) {
        wInfo.isOnline = false;
        wInfo.socketId = null;
      }
...
// Update live worker location & availability (REST fallback)
app.post('/api/worker/update-location', async (req, res) => {
  try {
    const { userId, latitude, longitude, formattedAddress, availability, skillType, serviceRadiusKm } = req.body;
    ...
    if (isNaN(lat) || isNaN(lng)) {
      return res.status(400).json({ success: false, message: 'Valid numerical coordinates required' });
    }
    ...
    res.json({ success: true, message: 'Worker location updated successfully' });
  } catch (err) {
...
// Static Map Image Proxy (high availability across Indian mobile networks & browsers)
app.get('/api/map/static', async (req, res) => {
  const lat = parseFloat(req.query.lat) || 19.1982;
  const lng = parseFloat(req.query.lng) || 72.9968;
  const zoom = parseInt(req.query.zoom) || 15;

  // 1. Primary: Yandex Static Map
  const yandexUrl = `https://static-maps.yandex.ru/1.x/?ll=${lng},${lat}&z=${zoom}&l=map&size=650,350`;
  try {
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3500);
    const response = await fetch(yandexUrl, { signal: controller.signal });
    ...
    // Fall back to Carto tile
  }

  // 2. Secondary: CartoDB Voyager raster tile
  try {
    ...
    const cartoUrl = `https://a.basemaps.cartocdn.com/rastertiles/voyager/${zoom}/${x}/${y}.png`;
    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), 3500);
    const cartoRes = await fetch(cartoUrl, { signal: controller.signal });
    clearTimeout(timeoutId);
    if (cartoRes.ok) {
  ...
  // 3. Fallback: High-res clean vector map placeholder
  res.setHeader('Content-Type', 'image/svg+xml');
  res.setHeader('Cache-Control', 'public, max-age=86400');
  return res.send(`
    <svg width="650" height="350" viewBox="0 0 650 350" xmlns="http://www.w3.org/2000/svg">
      <rect width="100%" height="100%" fill="#EEF2F6"/>
      <path d="M 0 90 Q 200 60, 420 110 T 650 100" stroke="#CBD5E1" stroke-width="20" fill="none"/>
      <path d="M 120 0 Q 150 170, 180 350" stroke="#CBD5E1" stroke-width="16" fill="none"/>
      <path d="M 0 240 Q 300 200, 650 250" stroke="#CBD5E1" stroke-width="22" fill="none"/>
      <path d="M 460 0 Q 480 180, 520 350" stroke="#CBD5E1" stroke-width="16" fill="none"/>
      <circle cx="325" cy="175" r="32" fill="#3B82F6" opacity="0.15"/>
      <circle cx="325" cy="175" r="14" fill="#2563EB"/>
...
app.post('/api/conversations/:conversationId/messages', async (req, res) => {
  try {
    ...
    const ioInstance = req.app.get('io');
    if (ioInstance) {
      const messageObj = populatedMessage ? (populatedMessage.toObject ? populatedMessage.toObject() : { ...populatedMessage }) : {};
      if (req.body.tempId) messageObj.tempId = req.body.tempId;
      ...
      // 1. Emit to the conversation room (both raw and prefixed IDs)
      ioInstance.to(conversationId).emit('receive_message', messagePayload);
      ioInstance.to(`chat:${conversationId}`).emit('receive_message', messagePayload);

      // 2. Emit to the personal rooms of other participants (for real-time badge / list updates in Chats list and Home)
      conversation.participants.forEach(pId => {
        const participantId = pId.toString();
        if (participantId !== senderId) {
          ioInstance.to(participantId).emit('receive_message', messagePayload);
          ioInstance.to(`user:${participantId}`).emit('receive_message', messagePayload);
...
// ==========================================
// AUTHORITATIVE JOB LIFECYCLE REST ENDPOINTS
// ==========================================
// 1. Fetch Authoritative Job State (HTTP fallback & reconnect sync)
...
// 3. Fetch Real Worker Completed Job Earnings & History
app.get('/api/workers/:workerId/earnings', async (req, res) => {
  try {
    const { workerId } = req.params;
    const User = mongoose.model('User');
    const Job = mongoose.model('Job');

    const worker = await User.findById(workerId).select('fullName totalEarnings completedJobsCount').lean();
    if (!worker) {
    ...
    const calculatedTotal = completedJobs.reduce((sum, j) => sum + (j.payment?.workerEarning || 0), 0);
    const totalEarnings = (worker.totalEarnings && worker.totalEarnings > 0) ? worker.totalEarnings : calculatedTotal;
    ...
    console.error('Error fetching worker earnings:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// 4. Fetch Authoritative Job History for User (Client or Worker)
app.get('/api/jobs/history/user/:userId', async (req, res) => {
  try {
    const { userId } = req.params;
...
// ========== WALLET / LEDGER / PAYMENT API ==========
const FinancialService = require('./services/FinancialService');
const financialServiceInstance = global.bookingDispatchEngine?.financialService || new FinancialService();

// GET /api/wallet/:workerId — Authoritative Wallet summary
app.get('/api/wallet/:workerId', authenticateJWT, async (req, res) => {
  try {
    const { workerId } = req.params;
    if (req.userId.toString() !== workerId.toString() && req.user?.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Unauthorized: You can only view your own wallet' });
    }
    const wallet = await financialServiceInstance.getWorkerWallet(workerId);
...
// GET /api/wallet/:workerId/transactions — Paginated ledger history
app.get('/api/wallet/:workerId/transactions', authenticateJWT, async (req, res) => {
  try {
    const { workerId } = req.params;
    if (req.userId.toString() !== workerId.toString() && req.user?.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Unauthorized: You can only view your own transactions' });
    }
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const result = await financialServiceInstance.getWorkerTransactionHistory(workerId, page, limit);
...
// POST /api/wallet/:workerId/withdraw — Request withdrawal (atomic with concurrency lock)
app.post('/api/wallet/:workerId/withdraw', authenticateJWT, async (req, res) => {
  try {
    const { workerId } = req.params;
    if (req.userId.toString() !== workerId.toString()) {
      return res.status(403).json({ success: false, message: 'Unauthorized: You can only withdraw from your own wallet' });
    }
    const { amount, bankName, accountLast4, idempotencyKey, payoutProvider } = req.body;
    if (!amount || !bankName || !accountLast4) {
      return res.status(400).json({ success: false, message: 'amount, bankName, and accountLast4 are required' });
    }
    const result = await financialServiceInstance.requestWithdrawal(
      workerId,
      Number(amount),
      { bankName, accountLast4, payoutProvider },
      null,
      idempotencyKey
    );
    if (!result.success) {
    ...
    const io = global.bookingDispatchEngine?.io || req.app.get('io');
    if (io && result.wallet) {
      const wPayload = {
        balance: result.wallet.balance,
        outstanding: result.wallet.outstanding,
        availableBalance: result.wallet.availableBalance,
        pendingWithdrawal: result.wallet.pendingWithdrawal,
        type: 'WITHDRAWAL_REQUESTED',
        amount: Number(amount),
        updatedAt: Date.now()
      };
      io.to(workerId.toString()).emit('wallet_updated', wPayload);
      io.to(`user:${workerId}`).emit('wallet_updated', wPayload);
    }
...
// GET /api/wallet/:workerId/withdrawals — Withdrawal history
app.get('/api/wallet/:workerId/withdrawals', authenticateJWT, async (req, res) => {
  try {
    const { workerId } = req.params;
    if (req.userId.toString() !== workerId.toString() && req.user?.role !== 'admin') {
      return res.status(403).json({ success: false, message: 'Unauthorized: You can only view your own withdrawals' });
    }
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const Withdrawal = require('./models/Withdrawal');
    const objectId = new mongoose.Types.ObjectId(workerId);
    const total = await Withdrawal.countDocuments({ $or: [{ workerId: objectId }, { labourId: objectId }] });
    const withdrawals = await Withdrawal.find({ $or: [{ workerId: objectId }, { labourId: objectId }] })
      .sort({ requestedAt: -1 })
      .skip((page - 1) * limit)
      .limit(limit)
      .lean();
    return res.json({ success: true, withdrawals, total, page, totalPages: Math.ceil(total / limit) });
  } catch (err) {
...
// POST /api/wallet/admin/payout/success — Mark payout successful & post double-entry disbursement
app.post('/api/wallet/admin/payout/success', authenticateJWT, async (req, res) => {
  try {
    const { withdrawalId, utrNumber, payoutProviderRef, payoutProvider } = req.body;
    if (!withdrawalId) {
      return res.status(400).json({ success: false, message: 'withdrawalId is required' });
    }
    const result = await financialServiceInstance.processWithdrawal(withdrawalId, {
      utrNumber,
      payoutProviderRef,
      payoutProvider
    });
    ...
    const io = global.bookingDispatchEngine?.io || req.app.get('io');
    if (io && result.wallet && result.withdrawal?.workerId) {
      const wId = result.withdrawal.workerId.toString();
      const wPayload = {
        balance: result.wallet.balance,
        outstanding: result.wallet.outstanding,
        availableBalance: result.wallet.availableBalance,
      ...
      io.to(wId).emit('wallet_updated', wPayload);
      io.to(`user:${wId}`).emit('wallet_updated', wPayload);
    }
    return res.json(result);
  } catch (err) {
    console.error('Error confirming payout success:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// POST /api/wallet/admin/payout/fail — Fail payout and atomically restore worker balance
app.post('/api/wallet/admin/payout/fail', authenticateJWT, async (req, res) => {
  try {
    const { withdrawalId, reason, failureCode } = req.body;
    if (!withdrawalId) {
      return res.status(400).json({ success: false, message: 'withdrawalId is required' });
    }
    const result = await financialServiceInstance.failWithdrawal(withdrawalId, reason, failureCode);
    if (!result.success) {
      return res.status(400).json(result);
    }
    return res.json(result);
  } catch (err) {
    console.error('Error recording payout failure:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// POST /api/wallet/admin/payout/cancel — Cancel withdrawal and release reserved balance
app.post('/api/wallet/admin/payout/cancel', authenticateJWT, async (req, res) => {
  try {
    const { withdrawalId, reason } = req.body;
    if (!withdrawalId) {
      return res.status(400).json({ success: false, message: 'withdrawalId is required' });
    }
    const result = await financialServiceInstance.cancelWithdrawal(withdrawalId, reason);
    if (!result.success) {
      return res.status(400).json(result);
    }
    return res.json(result);
...
// POST /api/wallet/:workerId/settle-outstanding — Manual outstanding settlement
app.post('/api/wallet/:workerId/settle-outstanding', authenticateJWT, async (req, res) => {
  try {
    const { workerId } = req.params;
    ...
    const result = await financialServiceInstance.settleOutstandingBalance(workerId, Number(amount), paymentRef);
    if (!result.success) {
      return res.status(400).json(result);
    }
    return res.json(result);
  } catch (err) {
    console.error('Error settling outstanding:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// GET /api/commission/config — Get commission config
app.get('/api/commission/config', async (req, res) => {
  try {
    const serviceType = req.query.serviceType || 'default';
...
const pricingEngineInstance = new PricingEngine({
  dispatchEngine: global.bookingDispatchEngine,
  activeWorkerLocations: global.activeWorkerLocations
});
global.pricingEngine = pricingEngineInstance;

// Seed default market rates on startup
if (mongoose.connection && mongoose.connection.readyState === 1) {
  MarketRate.seedDefaultRates().catch(err => console.error('[PricingEngine] Seed error:', err.message));
} else {
...
// GET /api/pricing/estimate — Location-aware dynamic price estimate
app.get('/api/pricing/estimate', async (req, res) => {
  try {
    ...
    const estimate = await pricingEngineInstance.calculateEstimate({
    ...
    console.error('Error calculating price estimate:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// POST /api/booking/dispatch — Dual-channel authoritative job creation & dispatch
app.post('/api/booking/dispatch', async (req, res) => {
  try {
    const data = req.body;
    ...
    // If job already running in memory, return existing state
    const existing = engine.jobs.get(data.jobId);
    if (existing) {
      return res.json({
        success: true,
        jobId: data.jobId,
    ...
    // Start job request through engine
    await engine.createAndStartJobRequest(null, data);
    const created = engine.jobs.get(data.jobId);

    return res.json({
      success: true,
      jobId: data.jobId,
      status: created ? created.status : 'SEARCHING',
      currentWave: created ? created.currentWave : 1,
      service: data.service,
      price: created ? created.price : data.price
    });
  } catch (err) {
    console.error('[API] /api/booking/dispatch error:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// ========== PAYMENT GATEWAY & ORDER ORCHESTRATION ==========
const PaymentService = require('./services/PaymentService');
const paymentServiceInstance = new PaymentService({
  financialService: financialServiceInstance,
  dispatchEngine: global.bookingDispatchEngine
});
global.paymentService = paymentServiceInstance;

// POST /api/payment/create-order — Authoritative payment order creation (JWT protected)
...
// POST /api/payment/create — Backwards compatible payment creation
app.post('/api/payment/create', async (req, res) => {
  try {
    ...
    const result = await paymentServiceInstance.createPaymentOrder({
      jobId,
      customerId: customerId || '000000000000000000000000',
      clientAmount: amount,
      idempotencyKey,
      description
    });
    return res.json({
      success: true,
      payment: {
        paymentId: result.gatewayOrderId,
        orderId: result.orderId,
        amount: result.amountInPaise / 100,
...
// POST /api/payment/verify-payment — Authoritative payment verification (JWT protected)
app.post('/api/payment/verify-payment', authenticateJWT, async (req, res) => {
  try {
    const { orderId, gatewayOrderId, gatewayPaymentId, gatewaySignature, jobId } = req.body;
    const customerId = req.userId;
    const result = await paymentServiceInstance.verifyAndConfirmPayment({
      orderId,
      gatewayOrderId,
      gatewayPaymentId,
      gatewaySignature,
...
// POST /api/payment/verify — Backwards compatible payment verification
app.post('/api/payment/verify', async (req, res) => {
  try {
    let { paymentId, orderId, gatewayOrderId, gatewaySignature, jobId } = req.body;
    if (!paymentId && !orderId && !gatewayOrderId && !jobId) {
    ...
    const result = await paymentServiceInstance.verifyAndConfirmPayment({
      orderId,
      gatewayOrderId,
      gatewayPaymentId: paymentId || gatewayOrderId,
      gatewaySignature,
      customerId: req.userId,
      jobId
    });
    return res.json({ success: true, verification: { verified: true, ...result } });
  } catch (err) {
    console.error('Error verifying payment:', err);
    return res.status(err.status || 400).json({ success: false, message: err.message, code: err.code });
  }
...
// POST /api/payment/webhook — Cryptographically verified gateway webhook
app.post('/api/payment/webhook', async (req, res) => {
  try {
    const rawBody = req.rawBody || (typeof req.body === 'string' ? req.body : JSON.stringify(req.body));
    const result = await paymentServiceInstance.processWebhook({
      rawBody,
      headers: req.headers
    });
...
// POST /api/payment/refund — Process refund (JWT protected)
app.post('/api/payment/refund', authenticateJWT, async (req, res) => {
  try {
    const { orderId, amountInPaise, reason } = req.body;
    if (!orderId) {
      return res.status(400).json({ success: false, message: 'orderId is required' });
    }
    const result = await paymentServiceInstance.processRefund({
      orderId,
      amountInPaise,
      reason
    });
...
app.get('/api/worker/active-job', authenticateJWT, async (req, res) => {
  try {
    ...
    console.error('[ActiveJob] Worker active job error:', err);
    return res.status(500).json({ success: false, message: err.message });
  }
});

// GET /api/customer/active-job — Authoritative active job check for customer
...
// POST /api/jobs/:jobId/rate — Authoritative two-sided rating submission
app.post('/api/jobs/:jobId/rate', authenticateJWT, async (req, res) => {
  try {
    const { jobId } = req.params;
    ...
    if (!completedStatuses.includes(job.status)) {
      return res.status(400).json({ success: false, message: 'Cannot rate a job that has not been completed' });
    }

    const currentUserIdStr = req.userId.toString();
    const isCustomer = job.clientId && job.clientId.toString() === currentUserIdStr;
    const isWorker = (job.workerId && job.workerId.toString() === currentUserIdStr) ||
                     (job.workerUserId && job.workerUserId.toString() === currentUserIdStr);

    if (!isCustomer && !isWorker) {
      return res.status(403).json({ success: false, message: 'Unauthorized: Only job participants can rate this job' });
    }

    const role = isCustomer ? 'CUSTOMER_TO_WORKER' : 'WORKER_TO_CUSTOMER';
    const toUserId = isCustomer ? (job.workerId || job.workerUserId) : job.clientId;

    if (!toUserId) {
      return res.status(400).json({ success: false, message: 'Recipient user ID missing on this job' });
    }

    // Enforce single rating per participant
    const existing = await Rating.findOne({ jobId, fromUserId: req.userId });
    if (existing) {
    ...
    if (isCustomer) {
      job.ratings.workerRating = ratingNum;
      job.ratings.workerReview = comment ? String(comment).trim() : '';
      job.ratings.customerRated = true;
      job.customerRated = true;
    } else {
      job.ratings.clientRating = ratingNum;
      job.ratings.clientReview = comment ? String(comment).trim() : '';
      job.ratings.workerRated = true;
      job.workerRated = true;
    }
    await job.save();

    // Recalculate recipient average rating and review count
    const allRatings = await Rating.find({ toUserId });
    if (allRatings.length > 0) {
      const sum = allRatings.reduce((acc, r) => acc + r.rating, 0);
      const avg = Math.round((sum / allRatings.length) * 10) / 10;
      await User.findByIdAndUpdate(toUserId, { rating: avg, reviews: allRatings.length });
    }
    ...
    if (engine) {
      ...
      engine.emitToJob(job, 'job_rated', payload);
      engine.io?.to(toUserId.toString()).emit('job_rated', payload);
      engine.io?.to(`user:${toUserId.toString()}`).emit('job_rated', payload);
    }
...
// GET /api/jobs/:jobId/ratings — Query two-sided rating status
app.get('/api/jobs/:jobId/ratings', async (req, res) => {
  try {
    ...
    const customerRated = Boolean(job.customerRated || ratings.some(r => r.role === 'CUSTOMER_TO_WORKER'));
    const workerRated = Boolean(job.workerRated || ratings.some(r => r.role === 'WORKER_TO_CUSTOMER'));
...
// ========== AUTHORITATIVE RECEIPT ENDPOINTS ==========
const { getBusinessConfig } = require('./config/businessConfig');

// GET /api/payment/receipt/:jobId — JSON Receipt Details
app.get('/api/payment/receipt/:jobId', authenticateJWT, async (req, res) => {
  try {
    ...
    if (!receipt) {
      ...
      // Early participant authorization check on job
      ...
      // Receipt is generated ONLY after authoritative payment confirmation
      ...
      // Resolve authoritative amount from PaymentOrder or confirmed Job
    ...
    // Security: Participant Authorization check on receipt
...
// GET /api/payment/receipt/:jobId/download — Printable HTML Invoice / Receipt
...
<head>
  ...
  <style>
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .line-items-table td.num { text-align: right; font-weight: 600; }
    .total-row {
      display: flex;
      justify-content: space-between;
      align-items: center;
...
<body>
  ...
  <div class="invoice-card">
    <div class="invoice-header">
      <div>
        <div class="brand-title">Allver</div>
        <div class="brand-sub">${documentHeading}</div>
      </div>
      <div style="text-align: right;">
        <div class="receipt-badge">PAID</div>
        <div style="font-size: 12px; margin-top: 6px; opacity: 0.9;">${receipt.receiptNumber}</div>
      </div>
    </div>
    <div class="invoice-body">
      <div class="meta-grid">
        <div class="meta-item">
          <label>Issued To (Customer)</label>
          <span>${receipt.customerName || 'Customer'}</span>
          ${receipt.customerPhone ? `<div style="font-size:12px; color:#64748b; margin-top:2px;">${receipt.customerPhone}</div>` : ''}
        </div>
        <div class="meta-item">
          <label>Service Professional</label>
          <span>${receipt.workerName || 'Service Partner'}</span>
          ${receipt.workerPhone ? `<div style="font-size:12px; color:#64748b; margin-top:2px;">${receipt.workerPhone}</div>` : ''}
        </div>
        <div class="meta-item">
      ...
      <table class="line-items-table">
        ...
        <tbody>
          <tr>
            <td>
              <strong>${receipt.serviceName || 'Home Service'}</strong>
              <div style="font-size:12px; color:#64748b; margin-top:2px;">Standard daily service rate</div>
            </td>
            <td class="num">₹${(receipt.baseAmount || receipt.totalAmount).toLocaleString('en-IN')}</td>
          </tr>
        </tbody>
      </table>
      ...
      <div class="total-row">
        <div>
          ...
          <div style="font-size: 12px; color: #64748b; margin-top: 2px;">
            Payment Method: <strong>${receipt.paymentMethod || 'Online UPI'}</strong>
            ${receipt.gatewayPaymentId ? ` | Ref: <code>${receipt.gatewayPaymentId}</code>` : ''}
          </div>
        </div>
        <div class="total-amount">₹${receipt.totalAmount.toLocaleString('en-IN')}</div>
      </div>
    </div>
    <div class="invoice-footer">
      ...
      <div>Support: ${receipt.businessDetails?.supportEmail || 'support@allver.app'}</div>
    </div>
  </div>
</body>
</html>`;
...
// ========== PERMANENT WORK & BOOKING HISTORY ==========
// GET /api/customer/work-history — Paginated booking history for customers
app.get('/api/customer/work-history', authenticateJWT, async (req, res) => {
  try {
    const Job = mongoose.model('Job');
...
// GET /api/worker/work-history — Paginated completed jobs history for workers
app.get('/api/worker/work-history', authenticateJWT, async (req, res) => {
  try {
    const Job = mongoose.model('Job');
    const page = Math.max(1, parseInt(req.query.page) || 1);
    const limit = Math.min(50, Math.max(1, parseInt(req.query.limit) || 20));
...
// ========== UNIFIED LABOUR EARNINGS API (DOUBLE-ENTRY BACKED) ==========

// GET /api/earnings/:labourId — Authoritative earnings summary derived from Ledger
app.get('/api/earnings/:labourId', authenticateJWT, async (req, res) => {
  try {
    const { labourId } = req.params;
    if (req.userId.toString() !== labourId.toString() && req.user?.role !== 'admin') {
      return res.status(403).json({ message: 'Unauthorized: You can only view your own earnings' });
    }
    const period = req.query.period || 'thisMonth';
    const wallet = await financialServiceInstance.getWorkerWallet(labourId);

    return res.json({
      totalEarnings: wallet.totalEarnings,
      grossEarnings: wallet.grossEarnings,
      netEarnings: wallet.netEarnings,
      totalPending: wallet.pendingWithdrawal,
    ...
    return res.status(500).json({ message: 'Error fetching earnings summary' });
  }
});

// GET /api/earnings/:labourId/payments — Authoritative job payment history from Ledger
app.get('/api/earnings/:labourId/payments', authenticateJWT, async (req, res) => {
  try {
    const { labourId } = req.params;
    if (req.userId.toString() !== labourId.toString() && req.user?.role !== 'admin') {
    ...
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const objectId = new mongoose.Types.ObjectId(labourId);

    const query = {
      workerId: objectId,
      account: 'Liabilities:WorkerPayable',
      entryType: 'CREDIT',
      type: { $in: ['ONLINE_EARNING', 'JOB_EARNING'] }
    };
    ...
    const payments = entries.map(e => ({
      _id: e._id.toString(),
      amount: e.amountInPaise / 100,
      type: e.type === 'ONLINE_EARNING' ? 'Online Job' : 'Job Payment',
      description: e.description,
      status: 'Paid',
      date: e.createdAt,
      paymentId: `ALV-PMT-${e._id.toString().slice(-6).toUpperCase()}`
    }));
...
// GET /api/earnings/:labourId/payments/:paymentId — Single payment detail from Ledger
app.get('/api/earnings/:labourId/payments/:paymentId', authenticateJWT, async (req, res) => {
  try {
    ...
    if (mongoose.Types.ObjectId.isValid(paymentId)) {
      entry = await Ledger.findById(paymentId).lean();
    }
    if (!entry) {
      entry = await Ledger.findOne({ postingId: paymentId }).lean();
    }
    ...
    const paymentDetail = {
      ...
      amount: entry.amountInPaise / 100,
      type: entry.type,
      description: entry.description,
      status: 'Paid',
      date: entry.createdAt,
      paymentId: `ALV-PMT-${entry._id.toString().slice(-6).toUpperCase()}`
    };

    return res.json({ payment: paymentDetail });
  } catch (err) {
    console.error('Error fetching payment detail:', err);
    return res.status(500).json({ message: 'Error fetching payment detail' });
  }
});

// POST /api/earnings/:labourId/withdraw — Unified withdrawal through FinancialService
app.post('/api/earnings/:labourId/withdraw', authenticateJWT, async (req, res) => {
  try {
    const { labourId } = req.params;
    if (req.userId.toString() !== labourId.toString()) {
      return res.status(403).json({ message: 'Unauthorized: You can only withdraw from your own account' });
    }

    const { amount, bankName, accountLast4, idempotencyKey } = req.body;
    if (!amount || !bankName || !accountLast4) {
      return res.status(400).json({ message: 'amount, bankName, and accountLast4 are required' });
    }
    ...
    if (!result.success) {
      return res.status(400).json({
        ...
        availableToWithdraw: result.availableForWithdrawal || 0
      });
    }
    ...
    if (io && result.wallet) {
      ...
      io.to(labourId.toString()).emit('wallet_updated', wPayload);
      io.to(`user:${labourId}`).emit('wallet_updated', wPayload);
    }
...
// GET /api/earnings/:labourId/withdrawals — Authoritative withdrawal history
app.get('/api/earnings/:labourId/withdrawals', authenticateJWT, async (req, res) => {
  try {
    const { labourId } = req.params;
    if (req.userId.toString() !== labourId.toString() && req.user?.role !== 'admin') {
      return res.status(403).json({ message: 'Unauthorized: You can only view your own withdrawals' });
    }
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;
    const objectId = new mongoose.Types.ObjectId(labourId);
    ...
    return res.status(500).json({ message: 'Error fetching withdrawal history' });
  }
});
