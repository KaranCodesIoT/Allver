...
const mapBackendPostToFeed = (bp: any, currentUserId?: string): PostData => {
  return {
    ...
    creator: {
      ...
      name: bp.creator?.fullName || 'User',
      role: bp.creator?.role || 'Professional',
      avatar: resolveAvatarUrl(bp.creator?.avatarUrl) || (bp.creator?.fullName ? `https://ui-avatars.com/api/?name=${encodeURIComponent(bp.creator.fullName)}&background=10B981&color=fff` : ''),
      location: bp.creator?.city || '',
      isVerified: Boolean(bp.creator?.isVerified),
    },
    timeAgo: 'Just now',
    bodyText: bp.description,
    images: bp.mediaUrls && bp.mediaUrls.length > 0 ? bp.mediaUrls.map((url: string) => resolveAvatarUrl(url)).filter((u: any): u is string => Boolean(u)) : [],
    likes: bp.likes || 0,
