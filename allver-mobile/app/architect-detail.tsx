...
export default function ArchitectDetailScreen() {
  ...
  useEffect(() => {
    ...
    const handleProfileUpdated = (data: any) => {
      if (data && data.userId === architectId && data.user) {
        console.log('[ArchitectDetail] Real-time profile update received:', data.user);
        setProfessionalData(data.user);
        if (data.user.followersCount !== undefined) {
          setFollowers(data.user.followersCount);
        }
    ...
    const handleUserStatsUpdated = (data: any) => {
      if (data && data.userId === architectId) {
        console.log('[ArchitectDetail] Real-time stats update received:', data);
      ...
      if (!user) {
        if (Platform.OS === 'web') {
          ...
          try {
            ...
            console.error('[ArchitectDetail] Error restoring user session from SecureStore:', e);
          }
        }
      }
      if (user) {
  ...
  useEffect(() => {
    if (architectId) {
      ...
      fetch(`${BACKEND_URL}/api/posts/user/${architectId}`)
        .then(res => res.json())
        .then(data => {
